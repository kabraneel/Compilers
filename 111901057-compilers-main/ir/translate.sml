...
fun compileExpr env  (Ast.Const x) t = ([MIPS.Inst(MIPS.Li(tempToReg(Temp.newtemp()),MIPS.Imm(x)))], env)
...
fun compileStmt env (Ast.Assign (x,y)) =
    ...
    | compileStmt env (Ast.Print     x) =
                        ...
                        in 
                            (res@[MIPS.Inst(MIPS.Move(MIPS.A0, tempToReg(newvar)))]@[MIPS.Inst(MIPS.Move(MIPS.V0, MIPS.Imm(1)))]@[MIPS.Inst(MIPS.Syscall)] , new_newenv)
                        end

fun compileprog env []          = [MIPS.Inst(MIPS.Move(MIPS.V0, MIPS.Imm(10)))]@[MIPS.Inst(MIPS.Syscall)]
